...
function renderScoreHud() {
  if (isCasualGame()) {
    ...
    scoreNode.innerText = `Ø§Ù…ØªÛŒØ§Ø²: ${state.game.score}`;
    scoreNode.style.display = "block";
    cubeCountNode.style.opacity = 0.65;
  }
  cubeCountNode.innerText = `Ù…Ú©Ø¹Ø¨ Ù‡Ø§ÛŒ Ø®Ø±Ø¯ Ø´Ø¯Ù‡ : ${state.game.cubeCount}`;
}
...
function renderMenus() {
  ...
  switch (state.menus.active) {
    ...
    case MENU_SCORE:
      ...
      if (isNewHighScore()) {
        highScoreLblNode.textContent = "Ø¨Ø§Ù„Ø§ØªØ±ÛŒÙ† Ø§Ù…ØªÛŒØ§Ø² Ø¬Ø¯ÛŒØ¯";
      } else {
        highScoreLblNode.textContent = `Ø¨Ø§Ù„Ø§ØªØ±ÛŒÙ† Ø§Ù…ØªÛŒØ§Ø² : ${formatNumber(
          getHighScore()
        )}`;
      }
